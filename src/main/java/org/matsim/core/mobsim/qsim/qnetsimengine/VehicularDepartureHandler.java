	...
	private void handleCarDeparture(double now, MobsimDriverAgent agent, Id<Link> linkId) {
		...
		if (vehicle == null) {
			if (vehicleBehavior == VehicleBehavior.teleport) {
				...
				if (vehicle == null && vehicleId.toString().endsWith("_access_uam_car")) {
					log.warn("Replacing access with egress vehicle for person " + agent.getId() + " and vehicle " + vehicleId);
					String new_id_string = vehicleId.toString().replaceAll("access", "egress");
					vehicleId = Id.create(new_id_string, Vehicle.class);
					vehicle = qNetsimEngine.getVehicles().get(vehicleId);
				}
				
				if ( vehicle==null ) {
					// log a maximum of information, to help the user identifying the cause of the problem
					log.warn("Null vehicle in car departure handling");
